...
> 由 brainstorming skill 在 PLANNING 之前的澄清门禁产出（可选）。存在即被 task-planning 优先采信。

```markdown
# 需求澄清
> task: {精炼后的任务描述}
> phase: BRAINSTORM（前置门禁，非正式 FSM 阶段）
...
## 已确认的隐性需求
{性能/并发、向后兼容、权限、错误与边界态等，逐条列出用户确认结果}

## 成功标准（验收点）
{可判定的验收条件——直接作为 TEST_FIRST 的测试依据与 spec 的 R 条目来源}

## 边界与失败模式
{枚举的边界情况/失败路径 + 是否在本次范围内}
...
## 遗留未决
{用户暂未决定、需在 PLANNING/ARCHITECTURE 中继续明确的点；无则写"无"}
```
